...
public class ProductDAO {
    ...
    public Double getProdCost(String prodCode) {
        ...
        try {
            String query = "SELECT CostPrice FROM product WHERE productid='" +prodCode+ "'";
            resultSet = statement.executeQuery(query);
            if (resultSet.next())
    ...
    public Double getProdSell(String prodCode) {
        ...
        try {
            String query = "SELECT SellingPrice FROM product WHERE Productid='" +prodCode+ "'";
            resultSet = statement.executeQuery(query);
            if (resultSet.next())
    ...
    public String getSuppCode(String suppName) {
        try {
            String query = "SELECT suppliercode FROM supplierinfo WHERE fullname='" +suppName+ "'";
            resultSet = statement.executeQuery(query);
    ...
    public String getCustCode(String custName) {
        try {
            String query = "SELECT customercode FROM supplierinfo WHERE fullname='" +custName+ "'";
            resultSet = statement.executeQuery(query);
            while (resultSet.next()) {
    ...
    public boolean checkStock(String prodCode) {
        try {
            String query = "SELECT * FROM stocks WHERE productid='" +prodCode+ "'";
            resultSet = statement.executeQuery(query);
            while (resultSet.next()) {
                flag = true;
            }
    ...
    public void addFunction(ProductDTO productDTO) {
        try {
            String query = "INSERT INTO product VALUES(?,?,?,?,?,?,?,null,1)";
...
/*
        ...
        if(checkStock(prodCode)) {
            try {
                String query = "UPDATE stocks SET Quantity=quantity+? WHERE productid=?";
                prepStatement = conn.prepareStatement(query);
    ...
    public void editProdDAO(ProductDTO productDTO) {
        try {
            String query = "UPDATE product SET ProductName=?,CostPrice=?,SellingPrice=?,Brand=? WHERE Productid=?";
            prepStatement = (PreparedStatement) conn.prepareStatement(query);
            ...
            String query2 = "UPDATE stocks SET quantity=? WHERE productid=?";
            prepStatement2 = conn.prepareStatement(query2);
    ...
    public void editPurchaseStock(String code, int quantity) {
        try {
            String query = "SELECT * FROM stocks WHERE productid='" +code+ "'";
            resultSet = statement.executeQuery(query);
            if(resultSet.next()) {
                String query2 = "UPDATE stocks SET quantity=quantity-? WHERE productid=?";
    ...
    public void deleteStock() {
        try {
            String query = "DELETE FROM stocks WHERE productid NOT IN(SELECT productid FROM purchaseinfo)";
            String query2 = "DELETE FROM salesinfo WHERE productcode NOT IN(SELECT Productid FROM product)";
    ...
    public void deleteProductDAO(int code) {
        try {
            String query = "DELETE FROM product WHERE Productid=?";
            prepStatement = conn.prepareStatement(query);
            prepStatement.setInt(1, code);

            String query2 = "DELETE FROM stocks WHERE productid=?";
            prepStatement2 = conn.prepareStatement(query2);
            prepStatement2.setInt(1, code);
    ...
    public void sellProductDAO(ProductDTO productDTO, String username) {
        ...
        try {
            String query = "SELECT * FROM stocks WHERE productid='" +productDTO.getProdCode()+ "'";
            resultSet = statement.executeQuery(query);
            while (resultSet.next()) {
                prodCode = resultSet.getString("productid");
            ...
            if (productDTO.getQuantity()>quantity)
                ...
                String stockQuery = "UPDATE stocks SET quantity=quantity-'"
                        +productDTO.getQuantity()
                        +"' WHERE productid='"
                        +productDTO.getProdCode()
                        +"'";
                String salesQuery = "INSERT INTO salesinfo(date,productcode,quantity,price,revenue,soldby)" +
                        "VALUES('"+productDTO.getDate()+"','"+productDTO.getProdCode()+"','"+productDTO.getQuantity()+"','"+productDTO.getSellPrice()+"','"+productDTO.getTotalRevenue()+"','"+username+"')";
    ...
    public ResultSet getQueryResult() {
        try {
            String query = "SELECT Productid,ProductName,CostPrice,SellingPrice,Brand FROM product";
            resultSet = statement.executeQuery(query);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
    ...
    public ResultSet getPurchaseInfo() {
        try {
            String query = "SELECT purchaseinfo.purchaseid,purchaseinfo.productcode,product.ProductName,purchaseinfo.Quantity,purchaseinfo.Totalcost " +
                    "FROM purchaseinfo INNER JOIN product " +
                    "ON product.Productid=purchaseinfo.productcode ORDER BY purchaseid;";
            resultSet = statement.executeQuery(query);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
    ...
    public ResultSet getCurrentStockInfo() {
        try {
            String query = """
                    SELECT stocks.productid,product.ProductName,
                    stocks.Quantity,product.CostPrice,product.SellingPrice
                    FROM stocks INNER JOIN product
                    ON stocks.productid=product.Productid;
                    """;
            resultSet = statement.executeQuery(query);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
    ...
    public ResultSet getSalesInfo() {
        try {
            String query = """
                    SELECT salesinfo.salesid,salesinfo.productcode,product.ProductName,
                    salesinfo.quantity,salesinfo.price,salesinfo.revenue,users.username AS Sold_by
                    FROM salesinfo INNER JOIN product
                    ON salesinfo.productcode=product.Productid
                    INNER JOIN users
    ...
    public ResultSet getPurchaseSearch(String text) {
        try {
            String query = "SELECT purchaseinfo.purchaseid,purchaseinfo.productcode,product.ProductName,purchaseinfo.Quantity,purchaseinfo.TotalCost " +
                    "FROM purchaseinfo INNER JOIN product ON purchaseinfo.productcode=product.Productid " +
                   // "INNER JOIN supplierinfo ON purchaseinfo.suppliercode=supplierinfo.suppliercode" +
                    "WHERE purchaseid LIKE '%"+text+"%' OR Productid LIKE '%"+text+"%' OR ProductName LIKE '%"+text+"%' " +
                   // "OR supplierinfo.fullname LIKE '%"+text+"%' OR purchaseinfo.suppliercode LIKE '%"+text+"%' " +
    ...
    public ResultSet getProdName(String code) {
        try {
            String query = "SELECT ProductName FROM product WHERE Productid='" +code+ "'";
            resultSet = statement.executeQuery(query);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
    ...
    String query = "SELECT SUM(Quantity) FROM stocks";
    try (
        ResultSet rs = statement.executeQuery(query)) {
        if (rs.next()) return rs.getInt(1);
    }
    ...
    String query = "SELECT SUM(revenue) FROM salesinfo";
    try (
        ResultSet rs = statement.executeQuery(query)) {
        if (rs.next()) return rs.getInt(1);
    }
